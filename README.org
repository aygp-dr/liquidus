...
v1.7 closes the v1 grind cycle. All 8 private validation repos land against successive spec revisions; every revision traces to a specific FEEDBACK.md observation. One major bump (v0 → v1.0) — the reified OAS was v1 classic, not v2 storefront/platform as v0 assumed. Everything after was additive. Major-bump reason (v0 → v1.0): reified OAS is the v1 Spree classic API, not the v2 storefront/platform split v0 assumed — path structure and auth model are both wire-breaking. See spec.org §Refutation conditions (spec-level).
